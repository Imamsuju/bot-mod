...
async function hytamkan(message, targetUser, reasonBan) {
    try {
        // Attempt to resolve the target as a guild member
        const targetMemberBan = typeof targetUser === 'string' 
            ? await message.guild.members.fetch(targetUser).catch(() => null)
        ...
        console.error(error);
        return message.reply(`❌ Mohon maaf, saya tidak bisa menghytamkan ${targetUser} karena posisinya lebih tinggi dari saya.`);
    }
}
